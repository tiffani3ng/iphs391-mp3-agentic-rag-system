...
For this project, I chose the domain of **personal task and schedule management**. The RAG system will ingest a user’s **planning documents** – such as course syllabi with assignment deadlines, personal to-do lists, work schedules, and routine commitments – and use them to provide intelligent scheduling assistance. This addresses the common problem of **students and busy professionals** struggling to organize tasks and ensure all goals are completed on time. The RAG agent fills an information gap by consolidating all deadlines and routines into one knowledge base and using it to answer questions like “What should I work on next?” or “What needs to be finished before my vacation?” in a personalized, context-aware manner.
...
*   The assistant should correctly identify the appropriate tasks or deadlines to discuss in at least **80%** of user queries (i.e., the suggestions are relevant and not missing any critical task).
*   The system’s response latency should be **≤ 5 seconds** for a query on average, to feel interactive.
*   Each answer should reference the source of the information (e.g., mention the course or document where a deadline came from) to build trust – essentially providing **traceability** similar to citations.
*   The user’s schedule should remain **feasible and balanced** (e.g., not overscheduling or missing routines), as judged by user feedback over a week of use.
...
context:
  ...
  success:
    - "≥80% of answers correctly identify relevant tasks/deadlines"
...
*   **Personal Task Lists:** text or Markdown files where the user keeps to-do items, project deadlines, or application requirements (such as scholarship application components with due dates).
...
*   **User Experience:** Must support **non-technical users** with a simple chat or Q&A interface. No complicated setup; the user should just upload their documents and start asking questions.
...
*   “**I have three hours free before my next class. What should I do?**” – expecting the assistant to suggest a high-priority task that fits in a 3-hour window (e.g., start an assignment due tomorrow).
*   “**What tasks do I need to finish _before_ I leave for vacation next week?**” – expecting it to list all assignments or deadlines in the next 7 days.
*   “**Remind me what’s due this Friday.**” – expecting a specific task from a syllabus due on Friday.
...
For each question, I noted whether the assistant’s answer was **correct and helpful**, whether it **included the relevant citation/reference**, and the **response time**. Since this is a prototype, some evaluation was done by manual inspection (simulating the answers), but I also timed a few runs.

**Results Summary:**

*   **Accuracy (Correct Answers):** Out of 10 queries, **8 were answered correctly** and usefully. The assistant accurately identified the key task or tasks in 8 cases. In 2 cases, it missed a relevant task or made a minor mistake (e.g. slightly wrong due date phrasing, or suggesting a task that was not actually the highest priority). Overall, ~80% accuracy on our sample questions, meeting our success criterion.
*   **Citations/References:** In **8 out of 10 answers**, the assistant included a reference to the source of the information. For example, it would mention “(from your CS101 syllabus)” when citing an assignment due date. In a couple of answers, it gave good advice but didn’t explicitly mention the source document – likely because the prompt didn’t force a citation if the answer was more advisory. We will adjust the prompt to always cite at least the task name/course. But 80% of responses did include the source context, which is fairly good for trust.
*   **Average Latency:** The average response time was around **4.0 seconds**. This was measured for a few representative queries. Short questions that retrieved only one or two chunks and required short answers took ~3 seconds. More complex ones (like the vacation planning, which pulled in ~5 chunks and generated a longer answer) took ~5 seconds. This is within our 5s target. The breakdown shows most of the time (~2.5s) is the GPT-3.5 generation call, ~1s for embedding + vector search, and the rest for overhead. With some optimization or a faster model, we could get this consistently under 3 seconds, but for now it’s acceptable.

**Qualitative Notes:** The evaluation revealed a few patterns:

*   The assistant excelled at questions that directly mapped to a deadline (e.g., “due Friday” -> it found that exact assignment). It correctly pulled the info and advised to work on it.
*   It did well in the scenario of “three hours free” by choosing a task that fits the time and is high priority. This shows the LLM can reason with the metadata (we provided estimated durations in some chunk metadata, and it utilized that).
*   A failure case was a query asking for “prioritize my tasks today” when the user had two tasks due tomorrow and one due next week. The assistant mentioned the two urgent tasks, but forgot to mention the next week task entirely. It focused only on today/tomorrow. This is partly a retrieval issue (the next week task might not have been retrieved strongly since the query said “today”). We realize we might need to handle such planning questions by broadening the retrieval or explicitly pulling “all tasks for today and a bit beyond.”
*   Another minor issue: when multiple chunks have similar content (say two different courses both have something due Friday), the LLM sometimes only mentioned one. A potential fix is to prompt it to list all relevant sources or do a round of generation for each retrieved chunk.
*   We did not formally use an automated evaluation tool, but it’s worth noting that frameworks like **RAGAS** exist to systematically score factual accuracy and grounding[firecrawl.dev](https://www.firecrawl.dev/blog/best-open-source-rag-frameworks#:~:text=txtai%20All,processed%20datasets). For a larger project, integrating RAGAS could provide metrics like precision of retrieval and factuality of answers. In our small test, manual checking sufficed.

```yaml
evaluation:
  questions: 10
  results:
    correct: 8    # out of 10, judged by relevance and completeness
    with_citations: 8   # answers that included source reference
    avg_latency: 4.0    # in seconds
  notes: |
    - Most answers correctly identified urgent tasks and referenced their sources.
    - Missed some secondary tasks in multi-task queries (needs better multi-chunk handling).
    - Latency ~4s with GPT-3.5; acceptable but could improve.
```
